...
private:
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
        ...
        if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
            ...
            (messageSeverity == 4096?"Error ":(messageSeverity == 256?"Warning ": (messageSeverity == 16?"Info":(messageSeverity == 1?"Verbose":"Max Flag Bit"))))
    ...
    VkShaderModule createShaderModule (const std::vector<char>& code) {
        ...
        if (result != VK_SUCCESS) throw std::runtime_error("failed to create a shader module");
